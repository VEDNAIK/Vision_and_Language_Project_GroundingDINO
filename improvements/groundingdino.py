...
def evaluate_on_subset(model, data_loader, device, subset_criteria):
    model.eval()
    for images, targets in data_loader:
        if subset_criteria(targets):
            # Perform evaluation
            pass

def subset_criteria(targets):
    pass
...
def iou_loss(pred_boxes, target_boxes):
    ...
    inter_width = torch.min(pred_boxes[:, 0] + pred_boxes[:, 2], target_boxes[:, 0] + target_boxes[:, 2]) - \
                  torch.max(pred_boxes[:, 0], target_boxes[:, 0])
    inter_height = torch.min(pred_boxes[:, 1] + pred_boxes[:, 3], target_boxes[:, 1] + target_boxes[:, 3]) - \
                   torch.max(pred_boxes[:, 1], target_boxes[:, 1])
    inter_area = inter_width.clamp(0) * inter_height.clamp(0)
